...
# Phase 1: Definition der Grundpunkte P and D
class Phase1(Scene):
    def construct(self):
        # Raster erzeugen
        gitter = NumberPlane(
...
# Phase 2: Hinzufuegen der Tiefenlinien (Orthogonalen)
class Phase2(Phase1):
    def construct(self):
        super().construct() # Laedt Phase 1
...
class Phase3(Phase2):
    def construct(self):
        ...
        querlinien = VGroup()
        for i in range(-5, 6):
            # Die perspektivische Hoehe berechnen
            y_pos = (3*i - 1) / (i + 9)
            
            # Die Breite an dieser Hoehe berechnen (Strahlensatz) 
            x_r = (15 - 5*y_pos) / 7
            x_l = -x_r
...
class Phase4(Phase3):
    def construct(self):
        ...
        # Sampling des Quadrats (dein bewaehrter Ansatz)
